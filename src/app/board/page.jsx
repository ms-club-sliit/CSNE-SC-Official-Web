  ...
  return (
    ...
    <div className="mx-10 flex flex-col items-center pt-10">
        ...
        <div className="md:flex flex-wrap justify-center items-baseline w-full hidden md:w-[70vw] top_board">
          {Top3Position.map((user) => {
            if (user.position === "PRESIDENT") {
              return <Teamcard key={user.id} height={440} width={440} name={user.name} image={user.image} position={user.position} link={user.link} />;
            } else {
              return <Teamcard key={user.id} height={170} width={170} name={user.name} image={user.image} position={user.position} link={user.link} />;
            }
          })}
        </div>
      )}
      ...
      <div className="grid md:grid-cols-5 gap-x-10 mx-10 place-items-baseline place-content-center">
        {TopPosition && TopPosition.length > 0 && (
          TopPosition.map((user) => (
            <Teamcard key={user.id} height={130} width={130} name={user.name} image={user.image} position={user.position} link={user.link} />
          ))
        )}
      </div>
