  ...
  return (
    <div className="">
      <div className="flex flex-col sm:flex-row gap-8 items-center">
        ...
        <div className="flex flex-col p-4 max-h-[400px] overflow-auto sm:flex-row gap-2 sm:gap-6">
          {data?.length > 0 ? (
            data.map((item) => (
              <div key={item.title}>
                <BlogVideoCard
                  icon={
                    item.icon === "RiBox3Line" ? (
                      <RiBox3Line className="text-5xl bg-[#F1F7FF] text-blue-600" />
                    ) : (
                      <IoWalletOutline className="text-5xl bg-[#FFF5DB] text-orange-300" />
                    )
                  }
                  title={item.title}
